...
model = torch.hub.load('C:/Users/space_lab/Desktop/git/STARFISH/Software/object_detection_training/model_training/yolov5-master', 
                       'custom', 
                       path='C:/Users/space_lab/Desktop/git/STARFISH/Software/object_detection_training/model_training/yolov5-master/runs/train/my_yolov5_model3/weights/best.pt', 
                       source='local', 
                       force_reload=True)
model.conf = 0.4  # Confidence threshold, tune as needed
...
while True:
    ...
    # potential gray scale example
    # gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    # gray_3ch = cv2.merge([gray, gray, gray])
    # results = model(gray_3ch)


    # red color detection using HSV
    ...
    # marker detection model inference
    # results = model(frame)
    detections = results.xyxy[0].cpu().numpy()  # shape: (num_detections, 6)

    circles = []  # list of (x, y, r, color)
    ...
    for *xyxy, conf, cls in detections:
        ...
        if sat_mean < 60 and val_mean > 180:
            color = "white"
        elif 0 <= hue_mean <= 10 or 160 <= hue_mean <= 179:
            color = "red"
        elif 40 <= hue_mean <= 80 and sat_mean >= 60:
            color = "green"
        else:
            color = "unknown"


        circles.append((cx, cy, r, color))


    binary = np.zeros_like(frame[..., 0])  # placeholder for now
    ...
    for marker_id, (tracker, last_seen, center) in trackers.items():
        ...
        if success:
            ...
            cv2.putText(frame, f"{color}", (x, y + r + 15), cv2.FONT_HERSHEY_SIMPLEX, 
            0.5, (255, 0, 0), 1)
    ...
    if len(trackers) < max_markers and circles:
        for i, (x, y, r, color) in enumerate(circles):
            ...
            trackers[f"{color}_marker_{i}"] = (tracker, 0, (x, y))
